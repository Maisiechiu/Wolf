...
public class ChangeScene : MonoBehaviour
{
    ...
    Scene m_scene;
    //public static AudioSource subMusic;

    static AudioSource mainMusic = null;
    ...
    void Start()
    {
        ...
        if(eightToThree){
            gameObject.transform.position = new Vector2(-17.7f,-5f);
            //subMusic.enabled = true;
        ...
        if(threeToEight){
            gameObject.transform.localScale = new Vector3(1, 1, 1); 
            //subMusic.enabled = false;
            mainMusic.enabled = false;
        }


        //subMusic = GameObject.Find("Audio Source").GetComponent<AudioSource>();
        //DontDestroyOnLoad(subMusic);
        mainMusic = GameObject.Find("mainAudioSource").GetComponent<AudioSource>();
        if(mainMusic==null){
            DontDestroyOnLoad(mainMusic);
        }
        // if(m_scene.name == "eightScene"){
        //     Destroy(mainMusic);
        //     //Destroy(ChangeScene.subMusic);
        // }
